    ...
    <form method="POST">
        <div id="bookings">
            <div class="booking-block mb-4 border rounded p-3 bg-white position-relative">
                ...
                <div class="mb-3">
                    <button name="action" value="availability" class="btn btn-outline-primary">Check Availability</button>
                    <button name="action" value="book" class="btn btn-outline-success">Book</button>
                </div>
            </div>
        </div>
...
def index():
    ...
    if request.method == 'POST':
        ...
        user = users[data['user'][0]]
        venue = venues[data['venue'][0]]
        date = data['date'][0]
        ...
        if action == "availability":
            ...
            try:
                ...
                slots = data.get("courtList", [])[0].get("slotInfo", [])
                ...
                result = f"<strong>Error fetching availability:</strong> {str(e)}"

        elif action == "book":
            num_slots = int(data['num_slots'][0])
            start_time = datetime.strptime(data['start_time'][0], "%H:%M:%S")
