...
users = {
    ...
    "Johnathan Mitchell": {
        ...
        "userId": "55bd04de-00a7-4e90-a16d-05b8d06daebf",
        "mobile": "3322097232",
        "email": "kolena3621@amirei.com"
    },
    "chagez khan": {
        "name": "chagez khan",
    ...
    <form method="POST">
        <div class="row g-3">
            <div class="col-md-6">
                ...
                <input type="text" name="cancel_booking_id" class="form-control" required>
            </div>
            <div class="col-md-6">
                <label class="form-label">Mobile Number</label>
                <input type="text" name="cancel_mobile" class="form-control" required>
            </div>
        </div>
...
def index():
    ...
    if request.method == 'POST':
        ...
        if 'cancel_action' in data:
            ...
            mobile = request.form['cancel_mobile']
            cancel_url = "https://playo.club/book-api/v1/booking/cancel/"
            payload = {"bookingId": booking_id, "mobile": mobile}

            try:
                res = requests.post(cancel_url, headers=headers, json=payload)
                if res.status_code == 200:
                    result += f"<strong>Booking ID {booking_id}</strong> cancelled successfully.<br>{res.text}<hr>"
                    success = True
                else:
                    result += f"<strong>Failed to cancel booking ID {booking_id}:</strong><br>{res.text}<hr>"
            except Exception as e:
                result += f"<strong>Error cancelling booking:</strong> {str(e)}<hr>"

            return render_template_string(form_template, users=users, venues=venues, result=result, success=success)

        # Handle availability/booking
